...
(function (g) {
  ...
  // ---------- راه‌حل پیش‌فرض: اگر موتور تابع رستر ندارد، از loadImage استفاده کن ----------
  function rasterizeFallback(svgString) {
    return false;
  }
    ...
    TEMPLATES.forEach(function (tpl) {
      ...
      card.addEventListener('click', function () {
        if (loadTemplate(tpl.id)) {
          ...
          // اگر rasterize در دسترس نیست، فقط اعلام کن
          if (g.Utils && g.Utils.toast) {
            g.Utils.toast('قالب به‌زودی اضافه می‌شود...', 'info');
          }
        }
      });
